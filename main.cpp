...
if(con.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_Y)){
	...
	mlpiston.set_value(matchloader);
}
...
typedef struct __attribute__((__packed__)) optical_rgb_s {
  ...
  double brightness;
	...
	// con.print(1, 0, "Integral | %2f", imu.get_heading()); //autstr //%s
//colorsorter

if(OpticalC.get_hue() > 7 && OpticalC.get_hue() < 30){
