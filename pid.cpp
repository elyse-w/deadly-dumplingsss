...
//     RB.brake();
// }
// Theta 90 = quarter-turn, 45 = 1/8th turn, 180 = half-circle).
void driveArcL(double theta, double radius, int timeout, int speed){
    ...
    while (true){
        double encoderAvgL = (LB.get_position() + LM1.get_position()) / 2;
        ...
        if (time2 % 50 == 0 && time2 % 100 != 0 && time2 % 150 != 0){
            ...
            con.print(1, 0, "rtarget: %f           ", float(rtarget));
        } else if (time2 % 150 == 0){
...
void driveArcLF(double theta, double radius, int timeout, int speed){
    ...
    while (true){
        ...
        double encoderAvgR = (RB.get_position() +  RM1.get_position()) / 2;
        double leftcorrect = -(encoderAvgL * 360) / (2 * pi * radius);
...
void driveArcR(double theta, double radius, int timeout, int speed){
    ...
    con.clear();
    //int timeout = 5000;
    ltarget = double((theta / 360) * 2 * pi * (radius + 565)); // * double(2) * pi * double(radius));
